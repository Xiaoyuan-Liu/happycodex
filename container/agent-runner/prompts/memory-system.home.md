...
#### HappyCodex 全局记忆（永久）→ 直接编辑 `/workspace/global/CLAUDE.md`

`/workspace/global/CLAUDE.md` 是 HappyCodex 的持久记忆文件，不等同于用户原生的个人 playbook 配置。
...
先读取当前内容，再**原地更新对应字段**（用最小编辑，不要整文件覆盖）。
...
> - 是 / 可能 → **HappyCodex 全局记忆**（编辑 `/workspace/global/CLAUDE.md`）
