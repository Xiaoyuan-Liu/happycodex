...
interface GroupState {
  ...
  restarting: boolean;
  // happycodex 决策：provider failover 作废 —— 上游此处的 selectedProviderId
  // （ProviderPool 选中的 profile，供 getStatus 展示与释放）已删除，codex 单引擎无此概念。
...
export class GroupQueue {
  ...
  registerProcess(
    ...
    opts: {
      ...
      agentId?: string;
      taskRunId?: string;
      // happycodex：上游 opts.selectedProviderId（provider failover）已删除。
    },
  ): void {
    const state = this.getGroup(groupJid);
    state.process = proc;
  ...
  getStatus(): {
    ...
    // happycodex：getStatus 条目里上游的 selectedProviderId（provider failover 展示）已删除。
    groups: Array<{
      jid: string;
      active: boolean;
      pendingMessages: boolean;
