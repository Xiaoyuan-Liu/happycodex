import type { StreamEvent, StreamDisplayLevel } from './stream-event.types.js';
  ...
  | {
      ...
      snapshot: {
        ...
        traceEvents?: Array<{
          ...
          toolUseId?: string;
          parentToolUseId?: string | null;
          // 上游为 StreamEvent['displayLevel']；happycodex 的 StreamEvent 无该字段，
          // 改引桥接文件里的同值联合（A0 搬迁触点）。
          displayLevel?: StreamDisplayLevel;
        }>;
        taskStates?: Record<string, unknown>;
        // 上游为 StreamEvent['contextAudit']（Claude 上下文注入审计，Claude 专属）；
        // codex 无对应概念，放宽为 unknown（A0 搬迁触点）。
