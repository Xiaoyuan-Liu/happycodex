 ...
 *
 * happycodex 适配（忠实搬迁自 upstream-happyclaw/src/agent-output-parser.ts）：
 *   - marker 常量来自 src/shared/stream-event.ts（<<<HAPPYCODEX_OUTPUT_*>>>，协议两端我们都拥有），
 *     此处 re-export 保持上游导出面不变（上游在本文件定义并导出）。
 *   - ContainerOutput 类型来自 src/container-output.ts（单一真相源；上游定义在 container-runner.ts，
 *     待 container-runner 搬迁后由其 re-export 兼容）。
 *   - Claude 限额嗅探（isProviderFailureResult / CLAUDE_LIMIT_*）随 provider failover 作废整段删除。
 *   - API_ERROR_PATTERNS 中 ANTHROPIC_* env 两条换成 codex/OpenAI 等价（OPENAI_API_KEY / codex login）。
 */
import fs from 'fs';
...
// happycodex：定义收敛到 shared/stream-event.ts（wrap 端 agent-runner.ts 同源），此处 re-export 对齐上游导出面。
...
export interface StdoutParserState {
  ...
  // tombstone（happycodex）：hasProviderFailureOutput 状态位随 provider failover 作废删除。
      ...
      while (
        ...
        try {
          ...
          if (parsed.status === 'success') {
            ...
            // tombstone（happycodex）：上游此处的 isProviderFailureResult 限额嗅探随 provider failover 作废删除。
          }
    ...
    waitForOutputChain(
      ...
      () => {
        ...
        ctx.resolvePromise({
          ...
          result: null,
          newSessionId,
          // tombstone（happycodex）：上游此处的 providerFailure 透传随 provider failover 作废删除。
        });
      },
    );
    return;
...
function parseLegacyOutput(ctx: CloseHandlerContext): void {
  ...
  try {
    ...
    if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
      ...
      // happycodex：noUncheckedIndexedAccess 下显式兜底空串（JSON.parse('') 抛错 → 走下方 catch，与上游行为一致）。
...
const API_ERROR_PATTERNS = [
  ...
  /\b(502|503|504|529)\b/,
  // happycodex：上游 ANTHROPIC_API_KEY / ANTHROPIC_AUTH_TOKEN 两条换成 codex/OpenAI 等价。
  /OPENAI_API_KEY/,
  /\bcodex\s+login\b/i,
  /\binvalid[_ ]?api\b/i,
...
// tombstone（happycodex）：上游此处的 Claude 账号限额嗅探整段（CLAUDE_LIMIT_PHRASE_PATTERNS /
// CLAUDE_LIMIT_RESET_PATTERN / CLAUDE_LIMIT_NOTICE_MAX_LEN / isProviderFailureResult 及其
// 文档注释，upstream agent-output-parser.ts:615-697）随 provider failover 作废删除。
 ...
 *
 * Used by container-runner for error classification/enrichment.
 * （happycodex：上游"report failure to ProviderPool"用途随 provider failover 作废。）
 */
