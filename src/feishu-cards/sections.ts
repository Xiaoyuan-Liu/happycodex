...
export function shortModel(model: string): string {
  ...
  // codex/gpt 模型名（happycodex adapt）：
  // "gpt-5.1-codex-max" → "codex-max-5.1", "gpt-5-codex" → "codex-5"
  const g = model.match(/gpt-(\d+(?:\.\d+)?)-codex(?:-([a-z0-9]+))?/i);
  if (g) {
    return g[2] ? `codex-${g[2].toLowerCase()}-${g[1]}` : `codex-${g[1]}`;
  }
