...
// happycodex 契约对齐：StreamEvent 无 displayLevel 字段，snapshot 类型引桥接联合
// （与 src/types.ts 的 WsMessageOut.stream_snapshot 同源，见 stream-event.types.ts）。
import type { StreamDisplayLevel } from './stream-event.types.js';
import {
  WEB_PORT,
  ...
  wss.on('connection', (ws, request: any) => {
    ...
    if (connSession && streamingSnapshots.size > 0) {
      ...
      for (const [jid, snap] of streamingSnapshots) {
        ...
        const baseJid = jid.includes('#agent:')
          ? (jid.split('#agent:')[0] ?? jid)
          : jid;
...
interface StreamingSnapshotEntry {
  ...
  traceEvents: Array<{
    ...
    toolUseId?: string;
    parentToolUseId?: string | null;
    // 上游为 StreamEvent['displayLevel']；happycodex 冻结契约无该字段，引桥接联合（同 types.ts）。
    displayLevel?: StreamDisplayLevel;
  }>;
  // 上游为 StreamEvent['contextAudit']（Claude 上下文注入审计）；codex 无对应概念，放宽为 unknown。
...
// 契约对齐（happycodex）：上游 kind 推导还覆盖 hook_* / memory_recall / compact_boundary /
// context_audit / permission_denied / raw_sdk_event 等事件并读取 skillName / title / summary /
// taskSummary / rawType / detail / taskId / parentToolUseId / displayLevel 字段——冻结契约
// （src/shared/stream-event.ts）无这些 eventType/字段，对应分支与字段随之裁剪；
// compact_partial（happycodex 扩展）归入 'memory' 档（语义最接近上游 compact_boundary）。
function pushTraceEvent(snap: StreamingSnapshotEntry, event: StreamEvent): void {
  ...
  const kind =
    event.eventType.startsWith('tool_') ? 'tool' :
    event.eventType.startsWith('task_') ? 'task' :
    event.eventType === 'compact_partial' ? 'memory' :
    'status';
  const title = event.statusText
    || event.toolName
    || event.eventType;
...
// 契约对齐（happycodex）：上游 snapshotTaskId 还读 parentToolUseId / taskId 字段，
// updateSnapshotTask 还处理 task_progress / task_updated / task_notification 与
// parentToolUseId 维度的子工具归属——冻结契约无这些 eventType/字段，整段裁剪为
// task_start 按 toolUseId 建档的最小骨架（W2 线报①后 codex 侧已无 task_start
// 生产者——turn/started 不再映射；骨架保留为上游契约兼容）。
// codex 的子代理增量走 agentScope='subagent' 维度（decorateScope），不挂 task 树。
function snapshotTaskId(event: StreamEvent): string | null {
  return event.eventType.startsWith('task_') ? event.toolUseId || null : null;
}
...
function updateSnapshotTask(snap: StreamingSnapshotEntry, event: StreamEvent): void {
  ...
  const task = snap.taskStates[taskId] || {
    id: taskId,
    title: event.toolInputSummary || 'Task',
...
function updateStreamingSnapshot(normalizedJid: string, event: StreamEvent): void {
  ...
  // W2 线报②：仅主会话 idle 收口——子代理 thread 的 thread/status/changed idle
  // （携 threadId → decorateScope 标 subagent）不清主快照（主轮可能仍在飞）。
  ...
  // 契约对齐（happycodex）：上游用 parentToolUseId 区分「子任务内」增量、用 skillName
  // 标 Skill 工具——冻结契约无这两个字段。codex 的子代理维度是 agentScope='subagent'
  // （session.decorateScope 注入），故主快照文本累积改用该维度过滤。
  switch (event.eventType) {
    case 'text_delta':
      if (event.text && event.agentScope !== 'subagent') {
    ...
    case 'task_start':
      pushRecentEvent(snap, {
        id: event.toolUseId || `task-${Date.now()}`,
        timestamp: Date.now(),
        text: `Task 启动: ${event.toolInputSummary || event.subagentType || 'Task'}`,
        kind: 'task',
      });
      break;

    // 契约对齐（happycodex）：上游此处还有 task_progress / task_notification /
    // hook_started / hook_progress / hook_response / memory_recall / compact_boundary /
    // context_audit / todo_update 等 case——冻结契约（src/shared/stream-event.ts）
    // 无这些 eventType，codex 引擎不产出，整段裁剪。hook 流（B3）字段已留在契约里
    // 但尚无独立 hook_* eventType；codex 压缩流走下方 compact_partial。
    ...
    // happycodex 扩展：上下文压缩局部摘要（语义最接近上游 compact_boundary → 'memory' 档）
    case 'compact_partial':
      pushRecentEvent(snap, {
        id: `compact-partial-${Date.now()}`,
        timestamp: Date.now(),
        text: event.compactReason === 'auto' ? '上下文自动压缩' : '上下文压缩',
        kind: 'memory',
      });
      break;
  }
