/**
 * 定时任务调度器 —— 忠实搬迁自 upstream-happyclaw/src/task-scheduler.ts（1227 行）。
 *
 * 与上游的差异（仅触点级，结构/逻辑逐字保留）：
 *   - StreamEvent import 路径与上游一致（./stream-event.types.js），该桥接文件指向
 *     happycodex 单一真相源 src/shared/stream-event.ts（冻结契约）。
 *   - noUncheckedIndexedAccess 严格索引修正 1 处（ensureTaskWorkspace 的 split()[0]）。
 *   - deps.onProcess 的 selectedProviderId 形参保留上游签名；happycodex 的
 *     runContainerAgent/runHostAgent 恒传 null（provider pool 已作废，tombstone 见
 *     container-runner.ts）。
 *   - 上游注释中的一处 mojibake（"��新"）修正为"更新"。
 */
...
function ensureTaskWorkspace(
  ...
  // （happycodex：noUncheckedIndexedAccess 严格索引修正——split() 至少一元素，?? '' 兜底）
...
async function runTaskInner(
  ...
  const finalizeRunLog = () => {
    ...
    // 必须等到 updateTaskAfterRun() 更新 next_run 后才能释放防重复屏障（#363）
