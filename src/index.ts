...
// tombstone（happycodex）：上游还 import CONTAINER_IMAGE——唯一消费者是
// clearSessionRuntimeFiles 的 docker 兜底 rm（已随 codex 等价实现删除）。
import {
  ASSISTANT_NAME,
  DATA_DIR,
  GROUPS_DIR,
...
// tombstone（happycodex）：上游还 import willClearSessionOnProviderSwitch（provider pool
// 主动切换预测器）——provider failover 已作废，container-runner 不再导出，两处消费分支
// （processGroupMessages / processAgentConversation 的"主动切换注入历史"）一并删除。
import {
  AvailableGroup,
  ContainerInput,
  ContainerOutput,
...
// happycodex：clearSessionRuntimeFiles 的 codex 等价实现（SessionStore 映射删除 + CODEX_HOME 会话状态清理）
...
/**
 * 契约对齐（happycodex，W2 线报③后）：StreamEvent.usage 已由 stream-mapper 的
 * normalizeTokenUsage 归一化成上游结构化形状（顶层 inputTokens/outputTokens/
 * cacheReadInputTokens/...，token 取 codex tokenUsage.last 增量、costUSD 恒 0——
 * billing token-only 决策；语义详见该 helper 注释）。此 helper 把 Record 窄化为
 * 消费面（卡片 usage 注脚 / usage_records / billing）所需的数字字段：主路径直读
 * 顶层归一化字段；同时保留 tokenUsage.last 嵌套读取作为防御性兼容（旧引擎进程
 * 信封中的历史透传形状），双读不冲突——归一化形状下嵌套分支恒 0 落到顶层。
 */
export function readStreamUsage(usage: Record<string, unknown>): {
  inputTokens: number;
  outputTokens: number;
  ...
  const readNum = (obj: unknown, key: string): number => {
  ...
  const last =
    typeof tokenUsage === 'object' && tokenUsage !== null
      ...
      : undefined;
  return {
    // 主路径：顶层归一化字段（mapper normalizeTokenUsage 产出）；
    // 兜底：tokenUsage.last 嵌套形状（旧引擎进程信封的历史透传）。
    inputTokens: readNum(usage, 'inputTokens') || readNum(last, 'inputTokens'),
    outputTokens: readNum(usage, 'outputTokens') || readNum(last, 'outputTokens'),
    cacheReadInputTokens:
      readNum(usage, 'cacheReadInputTokens') || readNum(last, 'cachedInputTokens'),
    cacheCreationInputTokens: readNum(usage, 'cacheCreationInputTokens'),
    costUSD: readNum(usage, 'costUSD'),
    durationMs: readNum(usage, 'durationMs'),
    numTurns: readNum(usage, 'numTurns'),
  };
}
  ...
  switch (se.eventType) {
    ...
    case 'tool_use_start':
      if (se.toolUseId && se.toolName) {
        ...
        // 契约对齐（happycodex）：上游此处还把 skillName / isNested / toolInput 经
        // setToolMeta 附给飞书卡片（技能标签 / 嵌套工具 / AskUserQuestion 面板）——
        // 冻结契约（src/shared/stream-event.ts）无这三个字段，codex 不产出，整段裁剪；
        // label 退化为 toolName（上游 skillName 优先）。
        session.pushRecentEvent(`🔄 ${se.toolName}`);
      }
      break;
    case 'tool_use_end':
      if (se.toolUseId) {
      ...
      break;
    // 契约对齐（happycodex）：上游此处还有 tool_result case（工具返回值时间线）——
    // 冻结契约无该 eventType（codex 工具结果不经 StreamEvent 透传），整段裁剪。
    case 'tool_progress':
      if (se.toolUseId && se.toolInputSummary) {
        session.updateToolSummary(se.toolUseId, se.toolInputSummary);
      }
      // 契约对齐（happycodex）：上游此处还把 tool_progress.toolInput 写入 setToolMeta
      // 驱动飞书 ASK 面板（AskUserQuestion）——冻结契约无 toolInput 字段，裁剪。
      break;
    case 'status':
      ...
      break;
    // 契约对齐（happycodex）：上游此处还有 hook_started / hook_response / hook_progress
    // （setHook 面板）与 todo_update（setTodos 面板）case——冻结契约无这些 eventType
    // （B3 hook 字段已留在契约里但尚无独立 hook_* 事件），整段裁剪；卡片的 hook/todos
    // 面板按缺省降级（控制器 API 保留，从不调用）。
    case 'task_start':
      // 契约对齐（W2 线报①后）：mapper 已不再为 turn/started 产 task_start，
      // codex 侧该事件当前无生产者；本分支保留上游结构（toolUseId 门控）以兼容
      // 上游契约，子代理面板由 tool_use_start(collabAgentToolCall) 驱动。
      if (se.toolUseId) {
        const label = 'Task';
        if (session instanceof StreamingCardController) {
          session.updateTask(se.toolUseId, {
            // 契约对齐（happycodex）：上游 title 取 taskDescription || toolInputSummary，
            // summary 取 se.summary——冻结契约无 taskDescription/summary 字段。
            title: se.toolInputSummary || 'Task',
            status: 'running',
      ...
      break;
    // 契约对齐（happycodex）：上游此处还有 task_progress / task_updated / task_notification
    // case（SDK Task 树的进度/补丁/终态 → updateTask 面板）——冻结契约无这些 eventType；
    // codex 子代理生命周期走 tool_use_start/end(collabAgentToolCall)，整段裁剪。
    case 'usage':
      if (se.usage) session.patchUsageNote(readStreamUsage(se.usage));
      break;
    // 契约对齐（happycodex）：上游此处还有 permission_denied / memory_recall /
    // compact_boundary / notification / prompt_suggestion / context_audit /
    // raw_sdk_event case——冻结契约无这些 eventType，codex 引擎不产出，整段裁剪。
    // codex 压缩流走下方 compact_partial（happycodex 扩展），语义最接近上游
    // compact_boundary（setSystemStatus + 时间线一条）。
    case 'compact_partial':
      session.setSystemStatus(
        se.compactReason === 'auto' ? '上下文自动压缩' : '上下文已压缩',
      );
      if (se.text) {
        session.pushRecentEvent(`compact: ${se.text.slice(0, 80)}`);
      }
      break;
    case 'init':
...
// 契约对齐（happycodex）：上游 getSessionClaudeDir（data/sessions/{folder}/.claude）+
// clearSessionRuntimeFiles（保留 settings.json 清空 + docker 兜底 rm）整段替换为
// session-files.ts 的 clearSessionFiles —— codex 等价物：删 SessionStore 的
// folder→threadId 映射（下次不再 resume）+ 清 per-folder CODEX_HOME
// （data/sessions/{folder}/.codex）下的会话状态（rollout 等），保留
// auth.json/config.toml/agents/hooks.json（对应上游保留 settings.json 的语义）。
// Docker 兜底不需要：CODEX_HOME 由主进程（host 侧）写入，权限恒一致。
// 保留 async 包装让 3 处 `await clearSessionRuntimeFiles(...)` 调用面与上游逐字一致。
...
async function handleCommand(
  ...
  // 严格索引修正（noUncheckedIndexedAccess）：split 至少产一个元素
...
async function handleRecallCommand(chatJid: string): Promise<string> {
  ...
  logger.info(
    ...
    '/recall: built transcript, calling codex exec',
  );

  // Try to summarize via codex exec（上游：Claude CLI）
  const summary = await summarizeWithCodex(transcript);
  if (summary) {
    logger.info(
...
/**
 * Summarize a conversation transcript（上游 summarizeWithClaude，经 Claude Agent SDK；
 * happycodex 改走 sdkQuery 的 codex exec 一次性非交互实现，调用面不变）。
 */
async function summarizeWithCodex(transcript: string): Promise<string | null> {
...
async function generateAndApplyLLMTitle(
  ...
  try {
    ...
    const cleaned = (raw
      .trim()
      .split('\n')[0] ?? '') // 严格索引修正（noUncheckedIndexedAccess）
      .replace(/^["'「『《【\[(]+|["'」』》】\])]+$/g, '')
      .trim()
      .slice(0, 20);
    if (!cleaned) return;
...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  // tombstone（happycodex）：上游此处还有 `else if (willClearSessionOnProviderSwitch(folder))`
  // 分支——provider pool 主动切换前向新 provider 注入历史上下文。provider failover 已
  // 作废（codex 单账号凭据，无池化选择/粘性绑定），整段删除。
  ...
  // 严格索引修正（noUncheckedIndexedAccess）：missedMessages 此处保证非空（上方无消息已早退）
  ...
  // tombstone（happycodex）：上游此处定义 pickRunningTaskForNotification（SDK Task
  // task_notification 的 ID 兜底映射）——唯一消费分支随 task_notification 裁剪而删除。
      ...
      async (result) => {
        try {
          ...
          if (result.status === 'stream' && result.streamEvent) {
            ...
            // 仅累积主 Agent 文本。子 Agent 的中间文本混入会污染飞书主卡片正文与
            // interrupt_partial。
            // 契约对齐（happycodex）：上游按 parentToolUseId（SDK Task 树）过滤——冻结
            // 契约无该字段；codex 的子代理维度是 agentScope='subagent'
            // （session.decorateScope 注入），与 Web 端 web.ts applyStreamEvent 同口径。
            if (
              result.streamEvent.eventType === 'text_delta' &&
              result.streamEvent.text &&
              result.streamEvent.agentScope !== 'subagent'
            ) {
                ...
                try {
                  ...
                  lastReplyMsgId = await sendMessage(chatJid, interruptedText, {
                    ...
                    messageMeta: {
                      ...
                      // 契约对齐（happycodex）：上游读 streamEvent.sessionId——冻结契约
                      // 无该字段，codex 的会话标识是 threadId（resume 语义等价）。
                      sessionId:
                        result.streamEvent.threadId || activeSessionId,
            ...
            // 契约对齐（happycodex）：上游识别 `task_start && toolUseId` 或
            // `tool_use_start && toolName==='Task'`（Claude SDK Task 工具）。codex 的
            // 子代理 spawn 面是 collabAgentToolCall item（PoC 实测，见 memory），
            // toolName 即 item.type；task_start 在 codex 侧已无生产者（W2 线报①：
            // turn/started 不再映射），该腿保留为上游契约兼容、不触发。
            // taskDescription 字段冻结契约无 → 退化 toolInputSummary。
            const se = result.streamEvent;
            if (
              (se.eventType === 'task_start' && se.toolUseId) ||
              (se.eventType === 'tool_use_start' &&
                se.toolName === 'collabAgentToolCall' &&
                se.toolUseId)
            ) {
              try {
                ...
                const desc = se.toolInputSummary || '';
                ...
                // 契约对齐（happycodex）：上游 else-if 用 taskDescription 刷新已存在
                // 记录的 name/prompt——冻结契约无该字段，裁剪（首次创建已写入摘要）。
                broadcastAgentStatus(
                  chatJid,
                  taskId,
                  'running',
            ...
            // 契约对齐（happycodex）：上游此处还有 task_notification 终态分支
            // （SDK Task 完成/失败 → updateAgentStatus + pickRunningTaskForNotification
            // 兜底映射）——冻结契约无该 eventType；codex 子代理终态由上方
            // tool_use_end(collabAgentToolCall) 分支落库，整段裁剪。

            // Persist token usage to the latest agent message + usage_records
            // 契约对齐（happycodex）：se.usage 为 codex 宽松透传，结构化数字经
            // readStreamUsage 窄化（见该 helper 注释：token 取增量、costUSD 恒 0）。
            if (se.eventType === 'usage' && se.usage) {
              const usageNums = readStreamUsage(se.usage);
              try {
                updateLatestMessageTokenUsage(
                  chatJid,
                ...
                logger.debug(
                  {
                    ...
                    inputTokens: usageNums.inputTokens,
                  },
                  'Token usage persisted',
                );

                // Update billing monthly usage
                // 契约对齐（happycodex）：上游开闸条件是 costUSD truthy——token-only
                // 计费下 costUSD 恒 0，改为「有任何 token 用量」即累计（updateUsage
                // 的 token 配额维度仍需累加；cost 维度恒 0 自然 no-op）。
                const ownerGroup = registeredGroups[chatJid];
                if (
                  ownerGroup?.created_by &&
                  (usageNums.costUSD ||
                    usageNums.inputTokens ||
                    usageNums.outputTokens)
                ) {
                  try {
                    const effective = updateUsage(
                      ownerGroup.created_by,
                      usageNums.costUSD,
                      usageNums.inputTokens || 0,
                      usageNums.outputTokens || 0,
                    );
                    deductUsageCost(
                      ownerGroup.created_by,
                      usageNums.costUSD,
  ...
  if (
    ...
    // 清除会话文件（happycodex：SessionStore 映射 + CODEX_HOME 会话状态，保留凭据/配置）
  ...
  if (!sentReply) {
    broadcastStreamEvent(chatJid, {
      ...
      statusText: 'idle',
      turnId: lastProcessed.id,
      // 契约对齐（happycodex）：上游字段名 sessionId——冻结契约的会话标识是 threadId。
      threadId: activeSessionId,
    });
  }
  commitCursor();
...
async function runAgent(
  ...
  try {
    ...
    const onProcessCb = (
      ...
      // tombstone（happycodex）：上游还透传 opts.selectedProviderId（provider failover
      // 维度，GroupQueue 已删该字段）；onProcessCb 形参保留让 runContainer/HostAgent
      // 的调用形状与上游一致（恒传 null）。
...
async function processAgentConversation(
  ...
  // Inject history when the SDK session is fresh.
  // tombstone（happycodex）：上游条件还 `|| willClearSessionOnProviderSwitch(folder, agentId)`
  // （provider pool 主动切换会在 runner 内清 session）——provider failover 已作废，删除。
  if (!sessionId) {
    const historyContext = buildRecentConversationHistoryContext(
      virtualChatJid,
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        intro:
          '检测到当前 agent 的底层模型 session 是新的（可能因为恢复失败或上下文被重置）。以下是 HappyClaw 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      prompt = historyContext.context + prompt;
    ...
    if (output.status === 'stream' && output.streamEvent) {
            ...
            const persistedMsgId = storeMessageDirect(
              ...
              true,
              {
                meta: {
                  ...
                  // 契约对齐（happycodex）：sessionId ← streamEvent.threadId（见上）。
                  sessionId:
                    output.streamEvent.threadId || currentAgentSessionId,
                  sourceKind: 'interrupt_partial',
                  finalizationReason: 'interrupted',
                },
              },
      ...
      if (
        ...
        try {
          ...
          // 契约对齐（happycodex）：usage 经 readStreamUsage 窄化（codex 宽松透传）。
          writeUsageRecords({
            userId:
              effectiveGroup.created_by ||
            ...
            usage: readStreamUsage(output.streamEvent.usage),
          });
        } catch (err) {
          logger.warn(
            { err, chatJid, agentId },
  ...
  try {
    ...
    const onProcessCb = (
      ...
      const containerName = executionMode === 'container' ? identifier : null;
      // tombstone（happycodex）：selectedProviderId 不再入 registerProcess（见主路径注释）。
      void selectedProviderId;
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          if (group.created_by) {
            ...
            const ownerGate = checkOwnerActive(owner);
            if (!ownerGate.allowed) {
              // 严格索引修正（noUncheckedIndexedAccess）：groupMessages 非空（循环入口已判）
          ...
          if (sendResult === 'sent') {
            ...
            // 严格索引修正（noUncheckedIndexedAccess）：messagesToSend 非空（sent 分支前提）
...
async function hasActiveCpuDescendants(pid: number): Promise<boolean> {
  ...
  try {
    ...
    for (const line of stdout.trim().split('\n')) {
      ...
      // 严格索引修正（noUncheckedIndexedAccess）：上行已保证 length >= 3
...
async function main(): Promise<void> {
  ...
  queue.setOnUnconsumedAgentIpc((groupJid: string, agentId: string) => {
    ...
    const baseChatJid = groupJid.includes('#agent:')
      ? groupJid.split('#agent:')[0] ?? groupJid
      : groupJid;
    const agent = getAgent(agentId);
  ...
  const schedulerDeps: import('./task-scheduler.js').SchedulerDependencies = {
    ...
    onProcess: (
      ...
      // tombstone（happycodex）：selectedProviderId 形参保留（scheduler 签名对齐上游），
      // 不再入 registerProcess（GroupQueue 已删 provider 维度）。
