...
// --- happycodex 决策：billing 首版 token-only（搬迁触点） ---
// codex app-server 不回传可靠的 USD 成本（上游依赖 Claude Agent SDK 的
// total_cost_usd），首版所有 USD 维度的「拦截判定」一律关闭，配额判定只走
// token 维度。被本开关关闭的判定有三处：
//   1. _checkQuotaInternal/checkWindow 的 cost 配额上限（日/周/月）
//   2. _checkQuotaInternal 的 cost 维度 warningPercent 计算
//   3. _checkBillingAccessInternal 的最低余额（minBalanceUsd）开闸
// costUSD 相关的记录/钱包/兑换码/扣费代码全部保留（cost 恒为 0 时自然 no-op），
// 待 cost 数据源就绪后把本开关置 true 即可恢复上游完整行为。
const USD_GATING_ENABLED: boolean = false;
...
function _checkBillingAccessInternal(
  ...
  // happycodex token-only：最低余额开闸是 USD 维度判定，首版关闭（见 USD_GATING_ENABLED）。
  if (USD_GATING_ENABLED && balance.balance_usd < minBalanceUsd) {
    return {
      allowed: false,
...
function _checkQuotaInternal(userId: string): QuotaCheckResult {
  ...
  const checkWindow = (
    ...
    // happycodex token-only：cost 配额上限是 USD 维度判定，首版关闭（见 USD_GATING_ENABLED）。
    if (USD_GATING_ENABLED && costQuota != null && costUsed >= costQuota) {
      return {
        allowed: false,
  ...
  // happycodex token-only：cost 维度的 warningPercent 与 cost 判定一同关闭，
  // 避免「cost 已 100% 但 allowed=true」的显示矛盾（见 USD_GATING_ENABLED）。
  let warningPercent: number | undefined;
  const percents: number[] = [];
  if (USD_GATING_ENABLED && plan.monthly_cost_quota != null && plan.monthly_cost_quota > 0)
  ...
  if (USD_GATING_ENABLED && plan.daily_cost_quota != null && plan.daily_cost_quota > 0)
  ...
  if (USD_GATING_ENABLED && plan.weekly_cost_quota != null && plan.weekly_cost_quota > 0)
