...
export async function executeSessionReset(
  ...
  // 2. Clear codex session state（happycodex：上游清 .claude/ 保留 settings.json，
  //    这里改为删 SessionStore 的 folder→threadId 映射 + 清 per-folder CODEX_HOME
  //    下的 rollout，保留 auth.json/config.toml —— 见 session-files.ts）
