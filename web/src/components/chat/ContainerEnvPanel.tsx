...
// tombstone（happycodex）：上游此面板还编辑 per-group Claude provider 覆盖
// （ANTHROPIC_BASE_URL / ANTHROPIC_AUTH_TOKEN 专属输入），随 provider failover
// 作废——schemas.ts ContainerEnvSchema 已收窄为仅 customEnv，后端 zod strip
// 这些字段，继续渲染只会造成静默假成功。面板有意收窄为 customEnv-only。
...
export function ContainerEnvPanel({ groupJid, onClose }: ContainerEnvPanelProps) {
  ...
  const handleClear = async () => {
    if (!window.confirm('确定要清空所有自定义环境变量并重建工作区吗？')) return;
    setClearing(true);
    const ok = await saveConfig(groupJid, { customEnv: {} });
    setClearing(false);
    if (ok) {
      setSaveSuccess(true);
      if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3 space-y-4">
        <p className="text-[11px] text-muted-foreground leading-relaxed">
          为当前工作区注入环境变量（模型通过 ANTHROPIC_MODEL 覆盖），仅对当前工作区生效。保存后工作区将自动重建。
        </p>
