...
| 编号 | 严重度 | 缺陷 | 位置 | 备注 |
|---|---|---|---|---|
| #F5 | high → **已修复** | C5 的 IPC send_message 跨重试去重对**绝大多数群是 no-op**：`isRetryDuplicateIpcSend` 用 `web:${folder}` / `folder` 查 retryCount，但 queue 真实 key 是 chatJid（普通 web 群 = `web:<uuid>`，folder 是独立人类可读名），二者不等 → inRetry 恒 false。仅 admin home（folder='main'→`web:main`==jid）碰巧命中 | `src/index.ts:682-688`（上游/main:739 同源）| **更新（2026-06-25）：已本地前置修复**，对齐上游 happyclaw 修法——`inRetry = getJidsByFolder(sourceGroup).some(jid => queue.getRetryCount(jid) > 0)`，folder→真实 chatJid 反查、任一处于重试即视窗口内，覆盖 web:<uuid> 与 IM 群。修法复用本仓 / 上游既有的 `getJidsByFolder` 惯用法（index.ts 多处 sibling/home 反查同款，行号随文件增删漂移故不固化）。为可单测（index.ts main() 加载即执行、不可被测试 import）已把该判定抽到 `src/ipc-dedup.ts`（`createIpcSendDedup` 工厂，沿用 ipc-paths.ts/task-routing.ts 范式），核心 `getJidsByFolder().some()` 逻辑不变；故 index.ts 调用点为工厂调用、与上游 inline 形态有别，同步时该 hunk 需手动对齐。单测 `tests/ipc-dedup-retry-key.test.ts` 钉死「真实 jid 的 retryCount 才决定 inRetry」契约（含回退成猜 key 即失败的回归守卫）。〔同步提示：本地上游快照 `upstream-happyclaw/`（804a9e0）此处**仍是 buggy 原版**；据本轮对 happyclaw GitHub 的 PR #568 评审，上游 main 已用 getJidsByFolder 修复，但该提交未拉入本地快照、无法在本仓核实，故未来同步此 hunk 大概率可顺接、但**不保证零冲突**，需对照上游真实补丁。〕语义保留「仅重试重放窗口内抑制」→ 合法周期任务相同文案 / 用户主动重发在重试轮外不受影响（fail-open）。〔覆盖范围（诚实限定）：本次仅覆盖**主群会话 IPC root**（基础 registered_groups.jid：web:<uuid> / feishu:oc_xxx）。conversation-agent（`#agent:`）与 task（`#task:`）子运行以**虚拟 JID**（web:<uuid>#agent:<id>）入队、retryCount 记在虚拟 JID 上，而 getJidsByFolder 只返回基础 jid → 这些子运行的重试重放去重仍为 no-op；属修复未尽残余面（fail-open、非回归），与上文「改键有过度抑制风险、需独立设计」一并留作后续，届时若覆盖虚拟 JID 须先评估对 agent 周期任务的过度抑制。〕〔已知权衡（本修复新引入·窄域）：folder 级判定（`getJidsByFolder().some()`）下，同一 folder 多 jid 共址时（如 admin home `web:main`+`feishu:oc_xxx` 共享 folder='main'）存在窄域跨 jid 过度抑制——jid A 处于重试轮期间，jid B 在 10min 指纹 TTL 内合法重发**完全相同文案**会被误抑制；这是上文「改键有过度抑制风险」的具体体现、folder 级重试检测的固有权衡，fail-open，需「多 jid 共址 + 同 chatJid 在 TTL 内见过同文案 + sibling 正在重试」三者同时成立才触发，极窄。注：旧实现 inRetry 近乎恒 false、从不抑制，故该过度抑制系本修复本地新引入（非继承）；不收敛到 per-run jid 精确判定，因 source≠target 的跨群发送需查 source folder 的重试态（folder 级才正确），待与上游真实补丁对照后再定。〕〔原决策（2026-06-15）：不主动修、待上游跟进；本轮按用户要求对核实为真的缺陷直接前置修复。〕 |
