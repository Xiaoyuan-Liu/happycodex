...
function validateV2Shape(node: unknown, path: string[] = []): string[] {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (
      ...
      // happycodex adapt：rgba 正则放宽逗号后空格 —— 上游快照 sections.ts 实际
      // 输出 'rgba(227, 242, 253, 1)'（带空格，飞书线上接受），上游测试正则却
      // 不允许空格，test/src 自相矛盾（上游跑同样会红）。对齐 src 实际输出。
      const ok =
        ALLOWED_COLOR_ENUMS.has(value) ||
        /^([a-z\-]+)-(50|100|200|300|400|500|600|700|800|900)$/.test(value) ||
        /^rgba\(\d+,\s?\d+,\s?\d+,\s?(0|1|0?\.\d+)\)$/.test(value);
      if (!ok) {
